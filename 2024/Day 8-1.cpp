	...
	for (size_t n = 0; n < 62; ++n) {
		for (size_t i = 0; i < antennas[n].size(); ++i) {
			for (size_t j = i + 1; j < antennas[n].size(); ++j) {
				row = (size_t)2 * antennas[n][i].first - antennas[n][j].first;
				col = (size_t)2 * antennas[n][i].second - antennas[n][j].second;
				if (row >= 0 && row < SIZE && col >= 0 && col < SIZE) antinode[row][col] = true;

				row = (size_t)2 * antennas[n][j].first - antennas[n][i].first;
				col = (size_t)2 * antennas[n][j].second - antennas[n][i].second;
				if (row >= 0 && row < SIZE && col >= 0 && col < SIZE) antinode[row][col] = true;
			}
		}
	}
