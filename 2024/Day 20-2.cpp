	...
	for (size_t row = 0; row < map.size(); ++row) {
		for (size_t col = 0; col < map[0].size(); ++col) {
			...
			for (int dr = -20; dr <= 20; ++dr) {
				for (int dc = -20 + std::abs(dr); dc <= 20 - std::abs(dr); ++dc) {
					rowto = row + dr;
					colto = col + dc;
					if (rowto < map.size() && colto < map[0].size()) {
						if (map[rowto][colto] == '#') continue;
						score = forwardtrack[row][col].score + (std::abs(dr) + std::abs(dc)) + backwardtrack[rowto][colto].score;
						if (score < basescore) {
