...
int main() {
	...
	const int WIDTH = xmax - xmin + 3;
	const int HEIGHT = ymax - ymin + 1;
	const int SAND_X = 500 - xmin + 1;
	const int SAND_Y = 0;
		...
		for (int j = 0; j < coords[i].size() - 1; ++j) {
			...
			if (xmax != 0)
				for (x = 0; x != xmax; x += (xmax > 0 ? 1 : -1))
					cave[coords[i][j].first - xmin + 1 + x][coords[i][j].second - ymin] = '#';

			if (ymax != 0)
				for (y = 0; y != ymax; y += (ymax > 0 ? 1 : -1))
					cave[coords[i][j].first - xmin + 1][coords[i][j].second - ymin + y] = '#';
		}
	}
