...
int main() {
	...
	for (std::vector<Instruction>::iterator it = instructions.begin(); it < instructions.end(); ++it) {
		crates[(*it).to - 1].insert(crates[(*it).to - 1].end(), crates[(*it).from - 1].end() - (*it).amount, crates[(*it).from - 1].end());
		crates[(*it).from - 1].erase(crates[(*it).from - 1].end() - (*it).amount, crates[(*it).from - 1].end());
	}
