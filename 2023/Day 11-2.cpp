	...
	for (int i = 0; i < galaxies.size(); ++i) {
		for (int j = i + 1; j < galaxies.size(); ++j) {
			...
			for (int rowpath = std::min(galaxies[i].first, galaxies[j].first) + 1;
				...
				if (emptyrows[rowpath]) addrows += 999999LL;
			}
			for (int colpath = std::min(galaxies[i].second, galaxies[j].second) + 1;
				colpath < std::max(galaxies[i].second, galaxies[j].second); ++colpath) {
				if (emptycols[colpath]) addcols += 999999LL;
			}
			answer += (long long)std::abs(galaxies[i].first - galaxies[j].first)
				+ (long long)std::abs(galaxies[i].second - galaxies[j].second)
				+ addrows + addcols;
		}
	}
