...
int rankHand(Hand h) {
	std::vector<int> count = std::vector<int>(12, 0);
	int jokers = 0;
	for (int i = 0; i < 5; ++i) {
		if (h.cards[i] == 0) ++jokers;
		else count[h.cards[i] - 1] += 1;
	}
	count.erase(std::remove(count.begin(), count.end(), 0), count.end());
	std::sort(count.rbegin(), count.rend());
	if (jokers == 5) return 7;
	if (count[0] + jokers == 5) return 7;
	if (count[0] + jokers == 4) return 6;
	if (count[0] + jokers == 3) {
...
int main() {
	...
	while (std::getline(file, line)) {
		...
		for (int i = 0; i < 5; ++i) {
			if (isdigit(line[i])) h.cards[i] = line[i] - '0' - 1;
			else switch (line[i]) {
			case 'T': h.cards[i] = 9;  break;
			case 'J': h.cards[i] = 0;  break;
